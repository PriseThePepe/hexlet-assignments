...
public class PostsController {
    ...
    public static void update(Context ctx) {
        ...
        try {
            var validName = ctx.formParamAsClass("name", String.class)
                    .check(value -> value.length() >= 2,"Название не должно быть короче двух символов")
                    .get();

            var validBody = ctx.formParamAsClass("body", String.class)
                    .check(value -> value.length() >= 10, "Пост должен быть не короче 10 символов")
                    .get();

            Post post = PostRepository.find(id).orElseThrow(()-> new NotFoundResponse("Post not found"));
            post.setName(validName);
            post.setBody(validBody);
            post.setId(id);
            PostRepository.save(post);
            ctx.redirect(NamedRoutes.postsPath());
        } catch (ValidationException e) {
            var name = ctx.formParam("name");
            var body = ctx.formParam("body");
            var post = PostRepository.find(id).orElse(null);
            post.setName(name);
            post.setBody(body);
