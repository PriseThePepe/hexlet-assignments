...
public class ProductsController {
    ...
    // BEGIN
    @PutMapping(path = "/{id}")
    public ProductDTO update(@PathVariable long id,@RequestBody ProductUpdateDTO updateData) {
        var product = productRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Product with id " + id + " not found"));
        updateDTOtoEntity(updateData,product);
        productRepository.save(product);
        return toDTO(product);
    }
    private Product updateDTOtoEntity(ProductUpdateDTO productUpdateDTO, Product product) {
